...
use crate::{
    entities::{attributes::Attributes, item::Item, update::UpdateData},
    protocol::{
...
impl Player {
        ...
        while remaining_stack_count > 0 {
            match self.inventory.find_first_free_slot() {
                Some(slot) => {
                    ...
                    self.inventory.set(slot, item, attributes);
    ...
    pub fn try_equip_item_from_inventory(
        ...
        if self.inventory.has_item_in_slot(destination_slot) {
            self.inventory
                .swap(from_slot, destination_slot, attributes);
        } else {
            self.inventory
                .move_item(from_slot, destination_slot, attributes);
        }
        ...
        if let Some(target_item) = maybe_target_item {
                ...
                && target_item_stack_count < target_item_template.max_stack_count
            {
                ...
                if stack_diff < moved_item_stack_count as i32 {
                    ...
                    self.remove_item(from_slot, attributes);
                }
            } else {
                self.inventory.swap(from_slot, to_slot, attributes);
            }

            InventoryResult::Ok
        } else if is_destination_gear_slot {
            // Moving the item from a bag to gear: equip it
            self.try_equip_item_from_inventory(from_slot, attributes)
        } else {
            // Moving the item to a bag (from gear or a bag): just move it
            self.inventory
                .move_item(from_slot, to_slot, attributes);
            InventoryResult::Ok
        }
    }
    ...
    pub fn try_split_item(
        ...
        match self.inventory.get2_mut(from_slot, destination_slot) {
            ...
            (Some(moved_item), None) => {
                ...
                self.inventory
                    .set(destination_slot, new_item, attributes);
                self.session.send(&packet).unwrap();

...
impl<S> WorldSocketState<S> {
    // TODO: Handle partial packet here?
    async fn read_socket(&mut self) -> Result<[u8; 1024], WorldSocketError> {
...
impl WorldSocketState<SocketOpened> {
    async fn send_challenge(
        ...
        packet.send_unencrypted(&mut self.socket).await?;
...
impl WorldSocketState<ServerSentAuthChallenge> {
    async fn handle_auth_session(
        ...
        let mut reader = Cursor::new(buf); // TODO: Implement generic parsing
        ...
        packet.send(&mut self.socket, &mut encryption).await?;
